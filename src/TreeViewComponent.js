...
const getFileIcon = (filename,itemType) => {
  ...
  const handleItemClick = async (node) => {
    try {
      // Call handleNameClick to get children for the clicked item
      if(node.children && node.children.length>0)
      {
        return;
      }
     
      const currentChildren = await handleNameClick(node.id, node.itemType_i, node.userId_s, node.name_s, 3);
  
      //node.children=currentChildren;
      node.children = currentChildren.filter(child => {
        return child.itemType_i !== 4;
      });
      node.defaultExpanded=true;
      
    } catch (error) {
      // Handle error if handleNameClick fails
  ...
  const buildTree = nodes => {
    return nodes.map(node => (
      <TreeItem
        key={`${node.id}-${node.label}`} // Ensure unique key
        nodeId={node.id} // Use nodeId instead of id for TreeItem component
        label={
          <React.Fragment>
            <Stack direction='row' spacing={3}>
              <div sx={{marginBottom: '0' }}>
            {getFileIcon(node.name_s, node.itemType_i)}
            </div>
            {node.name_s}
            </Stack>
          </React.Fragment>
