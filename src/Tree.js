...
function TreeNode({ node,handleNameClick}) {
  const [isExpanded, setIsExpanded] = useState(node.isexpand || false);

  const FILE_ICONS = {
    users:<GroupIcon style={{ fill: '#505AC9',fontSize:'20px' }}/>,
    user:<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 48 48">
    ...
    folder2: '📁',
    folder:<FolderIcon style={{ fill: '#ebb434',fontSize:'20px' }}/>,
...
const getFileIcon = (filename,itemType) => {
  ...
  const handleExpand = async () => {
    ...
    if(!isExpanded)
    {
        var children =  await handleNameClick(node.id,node.itemType_i,node.userId_s,node.name_s,1);
        node.children=children.filter((child)=>{return child.itemType_i!=4}).map(child=>{child.parentPath=[...node.parentPath,node]});
  ...
  return (
    <div>
      ...
      startIcon={!isExpanded ? <Stack direction='row' ><ArrowRightIcon />{getFileIcon(node.name_s,node.itemType_i)}</Stack> : <Stack direction='row' ><ArrowDropDownIcon />{getFileIcon(node.name_s,node.itemType_i)}</Stack>}
      variant='text'
    >
        {node.name_s}
      </Button>
      {isExpanded && node.children && (
        <ul style={{ paddingLeft: '20px',margin:'0px'}}>
          {node.children.map(childNode => (
             <TreeNode node={childNode} handleNameClick={handleNameClick} />
          ))}
        </ul>
      )}
    </div>
...
export default function Tree({treeData,handleNameClick,getFileIcon}) {
  return (
    <div>
      {treeData.map(node => (
        <TreeNode node={node} handleNameClick={handleNameClick} />
      ))}
    </div>
  );
}
